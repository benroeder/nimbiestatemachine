#!/usr/bin/env python3
"""Test disk in open tray recovery with separate connections."""
...
def create_error_state():
    """Create disk in open tray error state using old system, then close connection."""
    print("=" * 60)
    print("STEP 1: CREATING ERROR STATE (Disk in Open Tray)")
    print("=" * 60)
    
    print("Using old state machine to create disk in open tray error state...")
    old_sm = None
    try:
        ...
        if not hw_state["disk_available"]:
            print("❌ No disk available - cannot create disk in open tray error state")
            return False
        ...
        with old_sm.manual_operation():
            ...
            if hw_state["disk_in_open_tray"] and hw_state["tray_out"]:
                print("✅ Error state created successfully: Disk in open tray")
                return True
            else:
...
def test_pure_state_machine_recovery():
    ...
    try:
        ...
        if not (hw_state["disk_in_open_tray"] and hw_state["tray_out"]):
            print("❌ Disk is not in open tray - error state was not preserved")
            return False
        
        print("✅ Error state preserved: Disk is in open tray")
        
        # Test initialization sequence - this should handle the disk in open tray
        print("\nTesting initialization sequence...")
        print("The pure state machine should detect and handle the disk in open tray")
...
def main():
    """Main test function."""
    print("DISK IN OPEN TRAY RECOVERY TEST")
    print("=" * 80)
    print("Testing recovery from disk in open tray error state using separate connections")
    ...
    # Summary
    print("\n" + "=" * 80)
    print("DISK IN OPEN TRAY RECOVERY TEST SUMMARY")
    print("=" * 80)
    
    if recovery_success:
        print("✅ SUCCESS: Pure state machine successfully recovered from disk in open tray error state")
        print("✅ The initialization sequence properly handles disk in open tray during startup")
        return True
    else:
        print("❌ FAILED: Pure state machine could not recover from disk in open tray error state")
        return False
